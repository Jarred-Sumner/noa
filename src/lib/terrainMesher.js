...
function TerrainMesher() {
    ...
    this.meshChunk = function (
        ...
        var subMeshes = greedyMesher.mesh(
            voxels,
            mats,
            cols,
            ao,
...
function MeshBuilder() {
    ...
    this.build = function (chunk, meshDataList, ignoreMaterials) {
        ...
        for (let index = 0; index < results.matIDs.length; index++) {
            let id = results.matIDs[index];
            mats[index] = getTerrainMaterial(id, ignoreMaterials);
        }
    ...
    function mergeSubmeshes(meshDataList, mergeAll) {
        ...
        for (var i = 0; i < meshDataList.length; ++i) {
            ...
            if (!target) {
                ...
                var targetIndicesLength = target.indices.length;
                target.indices.length += mdat.indices.length;

                for (var j = 0, len = mdat.indices.length; j < len; ++j) {
                    target.indices[targetIndicesLength + j] =
                        mdat.indices[j] + indexOffset;
                }
    ...
    function mergeArrays(tgt, src) {
        var originalLength = tgt.length;
        tgt.length += src.length;
        for (var i = 0; i < src.length; i++) tgt[i + originalLength] = src[i];
    }
    ...
    function getTerrainMaterial(matID, ignore) {
        ...
        if (!materialCache[name])
            materialCache[name] = makeTerrainMaterial(matID);
    ...
    function constructMeshDataFromMasks(
        ...
        for (var k = 0; k < len2; ++k) {
            ...
            for (var j = 0; j < len1; j += w, n += w) {
                ...
                for (w = 1; w < len1 - j; ++w) {
                    if (!maskCompareFcn(n + w, mask, maskVal, aomask, ao))
                        break;
                }
                ...
                OUTER: for (h = 1; h < len2 - k; ++h) {
                    for (var m = 0; m < w; ++m) {
                        var ix = n + m + h * len1;
                        if (!maskCompareFcn(ix, mask, maskVal, aomask, ao))
                            break OUTER;
                    }
                }
